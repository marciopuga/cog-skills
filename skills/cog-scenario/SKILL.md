...
description: >
  ...
  memory data. Invoke with /cog-scenario or when foresight flags a candidate.
---
...
- `memory/cog-meta/scenarios/` (existing scenarios — check for duplicates or related active scenarios; resolved ones carry a `## Retrospective` worth reading before you predict again)
...
**Confidence**: <how likely this path is to play out as described, stated plainly with the reason — no false precision>
```
...
Body format (the L0 line goes right after the frontmatter so the cog-meta index can pick it up):
```markdown
<!-- L0: Scenario — <decision, max 80 chars> -->
# Scenario: <decision>
...
5. **Honest confidence** — say how likely each branch is and why. Read past retrospectives in `scenarios/`; if earlier scenarios were overconfident, say so and adjust.
...
Read the relevant memory files for the scenario topic, plus any resolved scenarios' retrospectives. Model the futures. Be honest about uncertainty.
